...
export const EducationUnitsScreen = ({ navigation }) => {
  ...
  const [bookmarkedEducationModuleData, setBookmarkedEducationModuleData] =
    useState([]);
  const [completedEducationModuleData, setCompletedEducationModuleData] =
    useState([]);
  ...
  useEffect(() => {
    const completedEducationModules = educationModuleCompletionData.filter(
      (module) => module.status === "completed"
    );
    const skippedEducationModules = educationModuleCompletionData.filter(
      (module) => module.status === "skipped"
    );
    const completedData = completedEducationModules?.map((module) =>
      educationModules.find((item) => item.id === module.module_id)
    );
    setCompletedEducationModuleData(completedData);
    const skippedData = skippedEducationModules?.map((module) =>
      educationModules.find((item) => item.id === module.module_id)
    );
  ...
  useEffect(() => {
    ...
    const educationBookmarks = bookmarks?.filter(
      (bookmark) => bookmark < firstMovementUnit
    );
    const data = educationBookmarks.map((bookmark) =>
      educationModules.find((item) => item.id === bookmark)
    );
