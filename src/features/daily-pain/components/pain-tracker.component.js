...
export const PainTrackerComponent = ({ navigation }) => {
  ...
  const graphData = dailyPainScores
    .map((score) => {
      return {
        score: score.score,
        date: formatDateNoYear(score.date_time_value),
      };
    })
    .reverse();
  ...
  return (
    <>
      ...
      <DesriptionMessageWrapper>
        <DescriptionMessage>
          Your daily pain score progress since you began on {startDate}.
        </DescriptionMessage>
        <DescriptionHelpMessage>
          Scroll to the right to see all of the data.
        </DescriptionHelpMessage>
        <DescriptionHelpMessage>
          To adjust today's pain score use the arrow in the upper left corner.
        </DescriptionHelpMessage>
      </DesriptionMessageWrapper>
      <ButtonSection>
        <JournalButton
