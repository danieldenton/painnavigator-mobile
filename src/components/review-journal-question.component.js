...
import {
  EditIntensity,
  EditSelect,
} from "../components/edit-intensity.component";
...
export const InputQuestion = ({ changeEntry, editing, entry }) => {
  ...
  return (
    <QuestionWrapper style={{ borderTopWidth: editing ? 0 : 0.5 }}>
...
export const ReviewOptionButton = ({ destination, option, navigation }) => {
  return (
    <QuestionWrapper style={{ borderTopWidth: 0.5 }}>
      <QuestionButtonPressable onPress={() => navigation.navigate(destination)}>
        <QuestionButtonText>{option}</QuestionButtonText>
        <Next />
